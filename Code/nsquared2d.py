...
y_combined = []
x_combined = []
knowledge_dim = ['Factual', 'Conceptual', 'Procedural', 'Meta-Cognitive']
...
if TEST == 'Combined':
    ...
    print('0s are:', count_0, '1s are:', count_1, '2s are:', count_2, '3s are:', count_3)
    #global y_combined
    print('FINAL VALUES ARE', len(x_combined), len(y_combined[0]))
    ...
    with codecs.open('datasets/COMBINED_Exercise_Questions_Results.csv', 'w', encoding="utf-8") as csvfile:
        ...
        csvwriter.writerow(['Questions', 'Manual Label', 'Gaussian', 'Linear', 'Polynomial', 'RBF'])
        for question, label_know in zip(questions[-110:], labels_know):
            csvwriter.writerow([question, knowledge_dim[int(label_know)], knowledge_dim[int(y_combined[0][count])], 
                            knowledge_dim[int(y_combined[1][count])], knowledge_dim[int(y_combined[2][count])], knowledge_dim[int(y_combined[3][count])] ])
            count += 1
