...
def run(tid, chunk, return_dict):    
	...
	print('\n')
	for query in chunk:
		if len(query[1].split()) < 5:
			j += 1
			continue

		query_bow = id2word.doc2bow(clean(query[1]).lower().split())
...
'''
queries = []
with open('datasets/%s_SO_Questions.csv' %subject) as f:
...
'''
with open('../%s/__LSA_Filtered_Questions.csv' %subject, 'w') as f:
